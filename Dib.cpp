					...
					CPalette* pPal)
{
	...
	lpSrcDib  = (LPSTR) ::GlobalLock((HGLOBAL) hDIB);// 锁定DIB	
	lpSrcDibBits = GetBits(lpSrcDib);// 找到DIB图象像素起始位置	
	...
	bSuccess = ::StretchDIBits(hDC,								// 设备环境句柄
							   ...
							   lpSrcDibRect->left,                // 源X坐标
							   lpSrcDibRect->top,                 // 源Y坐标
							   GetRectWidth(lpSrcDibRect),           // 源宽度
							   GetRectHeight(lpSrcDibRect),          // 源高度
							   lpSrcDibBits,                      // 指向dib像素的指针
							   (LPBITMAPINFO)lpSrcDib,         // 指向位图信息结构的指针
...
{	
	...
	lpSrcDib = (LPSTR) ::GlobalLock((HGLOBAL) hDIB);// 锁定DIB
	lpbminfo= (LPBITMAPINFO)lpSrcDib;			
	long wNumColors =GetColorNum(lpSrcDib);// 获取DIB中颜色表中的颜色数目	
	if (wNumColors != 0)
	{		
...
/*************************************************************************
 * 函数名称：GetBits(LPSTR lpSrcDib)
 * 函数参数:
 *   LPSTR lpSrcDib，指向DIB对象的指针
 * 函数类型:LPSTR 
 * 函数功能:计算DIB像素的起始位置，并返回指向它的指针
 ************************************************************************/

LPSTR CDib::GetBits(LPSTR lpSrcDib)
{
	return (lpSrcDib + ((LPBITMAPINFOHEADER)lpSrcDib)->biSize+GetPalSize(lpSrcDib));
//	return (lpSrcDib + *(LPDWORD)lpSrcDib+GetPalSize(lpSrcDib));
}

/*************************************************************************
 * 函数名称：GetWidth(LPSTR lpSrcDib)
 * 函数参数:
 *   LPSTR lpSrcDib，指向DIB对象的指针
 * 函数类型:DWORD
 * 函数功能:该函数返回DIB中图象的宽度
...
DWORD CDib::GetWidth(LPSTR lpSrcDib)
{
	return ((LPBITMAPINFOHEADER)lpSrcDib)->biWidth;//返回DIB宽度
}


/*************************************************************************
 * 函数名称：GetHeight(LPSTR lpSrcDib)
 * 函数参数:
 *   LPSTR lpSrcDib ，指向DIB对象的指针
 * 函数类型:DWORD
 * 函数功能:该函数返回DIB中图象的高度
...
DWORD CDib::GetHeight(LPSTR lpSrcDib)
{
	return ((LPBITMAPINFOHEADER)lpSrcDib)->biHeight;//返回DIB高度

}
/*************************************************************************
 * 函数名称：GetPalSize(LPSTR lpSrcDib)
 * 函数参数:
 *   LPSTR lpSrcDib，指向DIB对象的指针
 * 函数类型:WORD
...
WORD CDib::GetPalSize(LPSTR lpSrcDib)
{	
	return (WORD)(GetColorNum(lpSrcDib) * sizeof(RGBQUAD));// 计算DIB中调色板的大小
}

/*************************************************************************
 * 函数名称：GetColorNum(LPSTR lpSrcDib)
 * 函数参数:
 *   LPSTR lpSrcDib，指向DIB对象的指针
 * 函数类型:WORD
 * 函数功能:该函数返回DIB中调色板的颜色的种数
 ************************************************************************/
WORD CDib::GetColorNum(LPSTR lpSrcDib)
{				
		long dwClrUsed = ((LPBITMAPINFOHEADER)lpSrcDib)->biClrUsed;	// 读取dwClrUsed值	
		if (dwClrUsed != 0)
		{			
			return (WORD)dwClrUsed;// 如果dwClrUsed不为0，直接返回该值
		}		
		WORD wBitCount = ((LPBITMAPINFOHEADER)lpSrcDib)->biBitCount;// 读取biBitCount值		
...
/*************************************************************************
 * 函数名称：GetBitCount(LPSTR lpSrcDib)
 * 函数参数:
 *   LPSTR lpSrcDib，指向DIB对象的指针
 * 函数类型:WORD 
 * 函数功能:该函数返回DIBBitCount
 ************************************************************************/
WORD CDib::GetBitCount(LPSTR lpSrcDib)
{
	return ((LPBITMAPINFOHEADER)lpSrcDib)->biBitCount;// 返回位宽
}
...
HGLOBAL CDib::LoadFile(CFile& file)
{
	...
	/////////////////////////////////////////////////////////////////////////
	//给CDib类的成员变量赋值	
	lpSrcDib = (LPSTR) ::GlobalLock((HGLOBAL) m_hDib);// 锁定
	lpbminfo=(BITMAPINFO*)lpSrcDib;
	lpbmihrd=(BITMAPINFOHEADER*)lpSrcDib;
	lpRgbQuag=(RGBQUAD*)(lpSrcDib+lpbmihrd->biSize);
	int m_numberOfColors =GetColorNum((LPSTR)lpbmihrd);
	if (lpbmihrd->biClrUsed == 0)
       lpbmihrd->biClrUsed =m_numberOfColors;
	DWORD colorTableSize = m_numberOfColors *sizeof(RGBQUAD);
	lpSrcDibBits=lpSrcDib+lpbmihrd->biSize+colorTableSize;
	/////////////////////////////////////////////////////////////////////////
	
	if (file.Read(lpSrcDib, dwFileSize - sizeof(BITMAPFILEHEADER)) !=// 读像素

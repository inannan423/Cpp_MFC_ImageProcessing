...
IMPLEMENT_DYNAMIC(xianxinbianhuan, CDialogEx)

xianxinbianhuan::xianxinbianhuan(CWnd* pParent /*=nullptr*/)
...
xianxinbianhuan::~xianxinbianhuan()
{
}

void xianxinbianhuan::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(xianxinbianhuan, CDialogEx)
	ON_WM_LBUTTONDOWN()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
	ON_WM_PAINT()
...
// xianxinbianhuan 消息处理程序


void xianxinbianhuan::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void xianxinbianhuan::OnKillFocus(CWnd* pNewWnd)
{
...
void xianxinbianhuan::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	// 判断当前光标是否在绘制区域
	if (m_MouseRect.PtInRect(point))
...
void xianxinbianhuan::OnLButtonUp(UINT nFlags, CPoint point)
{
	// 当用户释放鼠标左键停止拖动
	if (m_bIsDraging)
	{
...
BOOL xianxinbianhuan::OnInitDialog()
{
	// TODO:  在此添加额外的初始化
	// 调用默认OnInitDialog函数
...
void xianxinbianhuan::OnKillfocusEditA()
{
	// 保存用户设置
	UpdateData(TRUE);
...
void xianxinbianhuan::OnKillfocusEditB()
{
	// 保存用户设置
	UpdateData(TRUE);
